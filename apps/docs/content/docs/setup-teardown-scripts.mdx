...
`copyFiles` runs during workspace bootstrap and copies matching relative paths from the main repository into the new worktree.
...
Config is resolved with this priority order for each key (`setup`, `teardown`, `run`, `copyFiles`):
...
The same merge behavior applies to `copyFiles`.
